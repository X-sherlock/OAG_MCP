---
name: compare-funds-by-metric
description: Compare multiple fund codes on one or more supported metrics for the same period through Java and ifund_all_info.
---

# 多基金指标比较

Java skill_id: `compare_funds_by_metric`。比较至少两只基金在同一周期下的一组支持指标。

执行示例：

```powershell
'{"fund_codes":["000001","000002"],"period":"1y","attributes":["return_rate","max_drawdown","sharpe_ratio"]}' | python script.py
```
...
先找到 `skill_id` 等于 `compare_funds_by_metric` 或 `compare-funds-by-metric` 的调用，并优先使用该调用的 `params`。只有 `params` 缺字段时，才从 OAG 的 `targets`、`targetInstances`、`facts`、`factRequirements`、`task.constraints`、`normalizedSemanticFrame` 或 `normalized_semantic_frame` 中补齐。不要从自然语言问题中猜值。

需要构造的业务 JSON：

- `fund_codes`: 从 `params.fund_codes`、多个 Fund target 或候选基金列表中提取，至少 2 只基金。
- `period`: 从参数、任务约束或事实约束中提取。没有显式 `period` 时使用 OAG/ontology 默认值 `1y`。
- `attributes`: 从参数或事实属性中提取，至少 1 个比较指标。
...
可比较字段遵循 Java `metricSpec`：固定指标、周期收益/风险指标和同类排名字段均可被 Java 识别；实际比较通常使用 `return_rate`、`max_drawdown`、`drawdown`、`volatility`、`standard_deviation`、`sharpe_ratio`。同一次调用的周期必须对所有周期型属性有效。

## 缺参与 unsupported 处理

缺少至少 2 个 `fund_codes` 或缺少 `attributes` 时不调用。`period` 缺失时用默认 `1y`；属性在该周期不支持时不调用并返回 unsupported。
