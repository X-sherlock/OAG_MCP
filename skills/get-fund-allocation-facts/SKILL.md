---
name: get-fund-allocation-facts
description: Query stock, bond, cash, fund, and other asset allocation ratios from ifundtest.ifund_all_info through Java.
---

# 获取基金资产配置事实

Java skill_id: `get_fund_allocation_facts`。查询 `ifund_all_info` 最新大类资产占比，不支持持仓明细或日期维度明细。

执行示例：

```powershell
'{"fund_code":"000004","attributes":["stock_asset_ratio","fund_asset_ratio"]}' | python script.py
```
...
先找到 `skill_id` 等于 `get_fund_allocation_facts` 或 `get-fund-allocation-facts` 的调用，并优先使用该调用的 `params`。只有 `params` 缺字段时，才从 OAG 的 `targets`、`targetInstances`、`facts`、`factRequirements`、`task.constraints`、`normalizedSemanticFrame` 或 `normalized_semantic_frame` 中补齐。不要从自然语言问题中猜值。

需要构造的业务 JSON：

- `fund_code`: 从匹配调用参数或 Fund target 中提取。
- `attributes`: 从参数或事实属性中提取；未指定时 Java 返回大类资产占比白名单。不要传 `report_date`。
...
可返回字段：`stock_asset_ratio`、`cash_asset_ratio`、`bond_asset_ratio`、`fund_asset_ratio`、`other_asset_ratio`。

`asset_total_value`、`asset_net_value`、`stock_name`、`stock_nav_ratio`、`bond_name`、`bond_nav_ratio`、`holding_industry` 等字段当前在 `ifund_all_info` 不可用，只能结构化 unsupported。前十大持仓、行业配置、报告期持仓不由本包提供。
