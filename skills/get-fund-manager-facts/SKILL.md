---
name: get-fund-manager-facts
description: Query up to five fund managers, tenure days, managed scale, term return, gender code, and management start date from ifundtest.ifund_all_info through Java.
---

# 获取基金经理事实

Java skill_id: `get_fund_manager_facts`。返回基金经理列表与任职相关事实。

执行示例：

```powershell
'{"fund_code":"000002"}' | python script.py
```
...
先找到 `skill_id` 等于 `get_fund_manager_facts` 或 `get-fund-manager-facts` 的调用，并优先使用该调用的 `params`。只有 `params` 缺字段时，才从 OAG 的 `targets`、`targetInstances`、`facts`、`factRequirements`、`task.constraints`、`normalizedSemanticFrame` 或 `normalized_semantic_frame` 中补齐。不要从自然语言问题中猜值。

需要构造的业务 JSON：

- `fund_code`: 从匹配调用参数或 Fund target 中提取。
- `attributes`: 可选；Java 返回每位经理的结构化字段。
...
可返回 manager 结构：`manager_name`、`gender_code`、`manager_tenure_days`、`manager_scale`、`manager_term_return`、`management_start_date`、`display_order`。最多读取 `ifund_all_info` 中 5 位经理槽位。经理简历和外部履历不在当前数据源中。
