---
name: get-fund-peer-ranking-facts
description: Query peer return, drawdown, volatility, and Sharpe ranking facts by fund and period from ifundtest.ifund_all_info through Java.
---

# 获取基金同类排名事实

Java skill_id: `get_fund_peer_ranking_facts`。按基金和周期查询同类排名相关事实。

执行示例：

```powershell
'{"fund_code":"000001","period":"1y","attributes":["peer_return_rank","peer_sharpe_rank"]}' | python script.py
```
...
先找到 `skill_id` 等于 `get_fund_peer_ranking_facts` 或 `get-fund-peer-ranking-facts` 的调用，并优先使用该调用的 `params`。只有 `params` 缺字段时，才从 OAG 的 `targets`、`targetInstances`、`facts`、`factRequirements`、`task.constraints`、`normalizedSemanticFrame` 或 `normalized_semantic_frame` 中补齐。不要从自然语言问题中猜值。

需要构造的业务 JSON：

- `fund_code`: 从匹配调用参数或 Fund target 中提取。
- `period`: 从参数、任务约束或事实约束中提取。没有显式 `period` 时使用 OAG/ontology 默认值 `1y`。
- `attributes`: 提取并只保留本 skill 的排名属性；未指定时 Java 默认返回 `peer_return_rank`、`peer_drawdown_rank`、`peer_sharpe_rank`。
...
可返回字段：`peer_return_rank`、`peer_drawdown_rank`、`peer_volatility_rank`、`peer_sharpe_rank`。

`peer_return_rank` 支持 `1w`、`1m`、`3m`、`6m`、`1y`、`2y`、`3y`、`4y`、`5y`；`peer_drawdown_rank`、`peer_volatility_rank`、`peer_sharpe_rank` 支持 `1m`、`3m`、`6m`、`1y`、`2y`、`3y`、`ytd`、`si`。

`rank`、`percentile`、`peer_risk_rank`、`peer_average` 属于 ontology 可识别但 `ifund_all_info` 当前不能直接返回的字段，只能作为 unsupported 处理。

## 缺参与 unsupported 处理

缺少 `fund_code` 时不调用；周期与请求属性没有交集时不调用并返回 `UNSUPPORTED_PERIOD`。
