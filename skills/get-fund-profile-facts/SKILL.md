---
name: get-fund-profile-facts
description: Query fund profile, status, risk level, type flags, size, dates, company name, and relation-style profile facts from ifundtest.ifund_all_info through Java.
---

# 获取基金画像事实

Java skill_id: `get_fund_profile_facts`。获取基金基础信息、状态、标签、公司等 `object_profile` 和部分关系式画像事实。

执行示例：

```powershell
'{"fund_code":"000001","attributes":["fund_name","risk_level","company_name"]}' | python script.py
```
...
先找到 `skill_id` 等于 `get_fund_profile_facts` 或 `get-fund-profile-facts` 的调用，并优先使用该调用的 `params`。只有 `params` 缺字段时，才从 OAG 的 `targets`、`targetInstances`、`facts`、`factRequirements`、`task.constraints`、`normalizedSemanticFrame` 或 `normalized_semantic_frame` 中补齐。不要从自然语言问题中猜值。

需要构造的业务 JSON：

- `fund_code`: 从匹配调用参数或 Fund target 中提取。
- `attributes`: 从参数或事实属性中提取；未指定时 Java 返回 profile 白名单中的全部字段。
...
可返回字段：`fund_code`、`fund_name`、`fund_short_name`、`fund_type`、`fund_status`、`risk_level`、`sale_status`、`purchase_status`、`redemption_status`、`pension_fund_flag`、`index_fund_flag`、`fof_flag`、`etf_flag`、`dividend_mode`、`fund_size`、`listing_date`、`inception_date`、`company_name`。

基金经理明细使用 `get_fund_manager_facts`；交易门槛使用 `get_fund_purchase_rules`；资产配置、持仓、费率、分红不属于本 skill。

## 缺参与 unsupported 处理

缺少 `fund_code` 时不调用。无法解析为 6 位基金代码或已知基金名称时返回缺参/澄清错误，不猜测基金。
