...
def main():
    ...
    try:
        ...
        if "attribute" not in payload and isinstance(attributes, list) and attributes:
            payload["attribute"] = attributes[0]
        ranking = payload.get("ranking")
        if isinstance(ranking, dict):
            if ranking.get("direction") is not None:
                payload.setdefault("direction", ranking["direction"])
            if ranking.get("limit") is not None:
                payload.setdefault("limit", ranking["limit"])
        endpoint = (
